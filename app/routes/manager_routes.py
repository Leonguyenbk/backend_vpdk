...
from app.models import OrgUnit, User, UserRole
...
def list_users_in_my_department():
    ...
    manager_org_unit = OrgUnit.query.get_or_404(manager.org_unit_id)

    # If manager is in "BAN GIÁM ĐỐC" (Director Board), show all users except self and admin
    if manager_org_unit.name and "ban giám đốc" in manager_org_unit.name.lower():
        users = (
            User.query.filter(
                User.id != manager_id,
